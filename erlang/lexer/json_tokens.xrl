%% https://www.erlang.org/doc/man/leex
Definitions. 
...
UnescapedChar = [^\"\\]
EscapedChar   = (\\\\)|(\\\")|(\\b)|(\\f)|(\\n)|(\\r)|(\\t)|(\\/)
Unicode       = (\\{HexDigit}{HexDigit}{HexDigit}{HexDigit})
Quote         = [\"]
Delim         = [\[\]:,{}]
Space         = [\n\s\t\r]
...
literal([$\\,$\\ | Rest]) -> [$\\|literal(Rest)];
literal([$\\,$/ | Rest]) -> [$/|literal(Rest)];
literal([$\\,$b | Rest]) -> [$b|literal(Rest)];
literal([$\\,$f | Rest]) -> [$f|literal(Rest)];
literal([$\\,$n | Rest]) -> [$n|literal(Rest)];
literal([$\\,$r | Rest]) -> [$r|literal(Rest)];
literal([$\\,$t | Rest]) -> [$t|literal(Rest)];
literal([$\\,$u,D0,D1,D2,D3 | Rest]) -> 
  Char = list_to_integer([D0,D1,D2,D3], 16),
  [Char|literal(Rest)];
literal([C| Rest]) -> [C|literal(Rest)];
literal([]) -> [].
